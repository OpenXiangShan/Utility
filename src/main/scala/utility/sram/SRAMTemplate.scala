...
class SRAMTemplate[T <: Data](
  ...
  private val rcg = if(withClockGate) Some(Module(new MbistClockGateCell(false))) else None
  private val wcg = if(!singlePort && withClockGate) Some(Module(new MbistClockGateCell(false))) else None
  ...
  private val (mbistBd, array, vname) = SramHelper.genRam(
    ...
    if(rcg.isDefined) rcg.get.out_clock else clock,
    if(rcg.isDefined) wcg.map(_.out_clock) else Some(clock),
    suffix = "",
    foundry = "Unknown",
    sramInst = "STANDARD",
    template = this
  ...
  val (ren, wen) = (io.r.req.fire, io.w.req.valid || resetState)
