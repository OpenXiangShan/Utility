...
object Helpers {
  ...
  implicit class ExpressionHelper(expression: Expression) {
    def mapExpr(f: Expression => Expression): Expression = expression match {
      case SubField(expr, name, tpe) => SubField(f(expr), name, tpe)
      case SubIndex(expr, value, tpe) => SubIndex(f(expr), value, tpe)
      case SubAccess(expr, index, tpe) => SubAccess(f(expr), f(index), tpe)
      case Mux(cond, tval, fval, tpe) => Mux(f(cond), f(tval), f(fval), tpe)
      case ValidIf(cond, value, tpe) => ValidIf(f(cond), f(value), tpe)
      case DoPrim(op, args, consts, tpe) => DoPrim(op, args.map(f), consts, tpe)
